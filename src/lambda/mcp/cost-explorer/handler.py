...
Tools (5 — omits get_today_date since date is injected into agent prompts):
...
def handle_get_dimension_values(event):
    ...
    try:
        response = ce.get_dimension_values(
            TimePeriod={"Start": start_date, "End": end_date}, Dimension=dimension
        )
...
def handle_get_tag_values(event):
    ...
    try:
        response = ce.get_tags(
            TimePeriod={"Start": start_date, "End": end_date}, TagKey=tag_key
        )
...
def handle_get_cost_and_usage(event):
    ...
    try:
        ...
        print(f"Cost Explorer API request params: {json.dumps(params, default=str)}")
        response = ce.get_cost_and_usage(**params)
        print(f"Cost Explorer API raw response: {json.dumps(response, default=str)}")
        ...
        for result in response.get("ResultsByTime", []):
            ...
            for group in result.get("Groups", []):
                period_result["groups"].append(
                    {"keys": group.get("Keys", []), "metrics": group.get("Metrics", {})}
                )
                # Sum up grand total from grouped results
                for metric_values in group.get("Metrics", {}).values():
                    grand_total += float(metric_values.get("Amount", 0))
            # Also sum from Total if no groups
            if not result.get("Groups") and result.get("Total"):
                for metric_values in result["Total"].values():
                    grand_total += float(metric_values.get("Amount", 0))
...
def handle_get_cost_and_usage_comparisons(event):
    ...
    try:
        ...
        # Use the first metric from the requested metrics list
        metric_key = metrics[0] if isinstance(metrics, list) else metrics

        current_costs = {}
        for result in current_response.get("ResultsByTime", []):
            for group in result.get("Groups", []):
                key = group["Keys"][0]
                amount = float(group["Metrics"].get(metric_key, {}).get("Amount", 0))
        ...
        return {
            ...
            "group_by": group_by,
            "comparisons": comparisons,  # Return all — agent decides what to display
...
def handle_get_cost_forecast(event):
    ...
    if not start_date or not end_date:
        ...
        end_date = (
            (next_month.replace(day=1) + timedelta(days=31))
            .replace(day=1)
            .strftime("%Y-%m-%d")
        )
            ...
            "forecast_by_time": [
                {
                    ...
                    "prediction_interval_lower": item.get(
                        "PredictionIntervalLowerBound"
                    ),
                    "prediction_interval_upper": item.get(
