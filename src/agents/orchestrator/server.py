"""Generic orchestrator agent — serves any agent with type 'orchestrator' via HTTP.

Reads ``AGENT_NAME`` from the environment, loads its configuration from
``hierarchy.json``, and calls ``create_mid_level_agent()`` to wire up
registry-based child discovery and HTTP entrypoint.

Usage (Docker CMD)::

    CMD ["opentelemetry-instrument", "python", "-m", "agents.orchestrator.server"]
"""

from __future__ import annotations

import json
import logging
import os

from agents.shared.agent_base import create_mid_level_agent

logger = logging.getLogger(__name__)


def _load_agent_config(agent_name: str) -> dict:
    """Load agent configuration from hierarchy.json."""
    hierarchy_path = os.path.join(os.path.dirname(__file__), "..", "hierarchy.json")
    with open(hierarchy_path, encoding="utf-8") as f:
        hierarchy = json.load(f)
    if agent_name not in hierarchy:
        raise ValueError(f"Agent '{agent_name}' not found in hierarchy.json")
    return hierarchy[agent_name]


# ---------------------------------------------------------------------------
# Configuration from environment
# ---------------------------------------------------------------------------
AGENT_NAME = os.environ.get("AGENT_NAME")
if not AGENT_NAME:
    raise RuntimeError(
        "AGENT_NAME environment variable is required. "
        "Set it to the agent key in hierarchy.json (e.g., 'finops-agent')."
    )

config = _load_agent_config(AGENT_NAME)

# ---------------------------------------------------------------------------
# BedrockAgentCoreApp (via shared mid-level agent factory)
# ---------------------------------------------------------------------------
app, entrypoint = create_mid_level_agent(
    agent_name=AGENT_NAME,
    prompt_template=config["prompt"],
    model_id=config.get("model", ""),
)

if __name__ == "__main__":
    app.run()
